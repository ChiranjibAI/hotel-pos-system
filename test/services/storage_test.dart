...
void main() {
  ...
  group('Storage', () {
    ...
    test('#add', () async {
      await storage.add(.menu, 'hi', {'a': 'b'});
      final result = await storage.get(.menu, 'hi');
    ...
    test('#get - all', () async {
      await storage.add(.menu, 'hi', {'a': 'b'});
      await storage.add(.menu, 'there', {'b': 'c'});
      final result = await storage.get(.menu);
      expect(result, {
        'hi': {'a': 'b'},
        'there': {'b': 'c'},
      });
    ...
    test('#set', () async {
      Future<void> verifyResult(Map expected) async {
        final result = await storage.get(.menu, 'a');
        expect(result, expected);
      }

      await storage.add(.menu, 'a', {'old': 'value'});
      await storage.set(.menu, {
        'a': {'b': 'c'},
      });
      verifyResult({'old': 'value', 'b': 'c'});

      // delete field
      await storage.set(.menu, {'a.b': null});
      verifyResult({'old': 'value'});

      // delete all
      await storage.set(.menu, {'a': null});
      verifyResult({});

      // must be map
      await storage.add(.menu, 'a', {'b': 'c'});
      await storage.set(.menu, {'a.c': 2, 'a': 'b'});
      verifyResult({'c': 2, 'b': 'c'});
    });

    test('#setAll', () async {
      await storage.setAll(.menu, {
        'a': {'b': 'c', 'd': 'e'},
        'b': {'a': 'a'},
      });
      await storage.setAll(.menu, {
