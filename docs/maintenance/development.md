...
keytool -genkey -v -keystore android/app/my-jks.jks \
  -alias hotel_pos_system \
  -keyalg RSA \
  -keysize 2048 \
  -validity 10000 \
  -storepass hotel_pos_system \
  -dname 'CN=hotel_pos_system, OU=hotel_pos_system, O=hotel_pos_system, L=Unknown, ST=Unknown, C=Unknown'
```
...
- `/android/key.properties`: This file tells the build process where your key
  ...
  - `keyAlias=hotel_pos_system`
  - `keyPassword=hotel_pos_system` (If not specifically set, it defaults to the same as storePassword)
  - `storeFile=my-jks.jks`
  - `storePassword=hotel_pos_system`
...
printf "keyAlias=%s\nkeyPassword=%s\nstoreFile=%s\nstorePassword=%s" \
  ...
  'hotel_pos_system' \
  'my-jks.jks' \
  'hotel_pos_system' > android/key.properties
```
...
$ keytool -list -keystore android/app/my-jks.jks --storepass hotel_pos_system
